    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <SafeAreaView style={s.bottomSafe} edges={['bottom']}>
        <View style={s.bottomBar}>
          <TouchableOpacity style={s.pdfBtn} onPress={() => handleSubmit('pdf')} disabled={!!submitting || items.length === 0 || !!createdInvoice}>
            {submitting === 'pdf' ? <ActivityIndicator size="small" color="#333" /> : <><Ionicons name={createdInvoice ? 'checkmark-circle' : 'document'} size={16} color="#333" /><Text style={s.pdfBtnText}>{createdInvoice ? 'Created' : 'Create'}</Text></>}
          </TouchableOpacity>
