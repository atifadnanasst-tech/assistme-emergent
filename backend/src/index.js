...
- record_opening_balance_receivable: use when owner declares a pre-existing balance the customer owes them. Use when owner says "[customer] owes me [amount]", "[customer] ka opening balance [amount] hai", "opening balance [amount] kar do". Extract amount only. Do NOT try to judge whether this customer is new or has prior transactions -- you do not have access to their transaction history, only to recent chat messages, which are NOT reliable evidence of real invoices/payments/bills. Always extract this action when the phrasing matches; a separate, deterministic backend check (which DOES have real transaction data) will independently decide whether to allow or reject it.
- record_opening_balance_payable: same as record_opening_balance_receivable but for the OPPOSITE direction -- the owner owes THIS customer/entity. Use when owner says "I owe [customer] [amount]", "[customer] ko [amount] dena hai, opening balance", "[customer] se [amount] ka maal liya tha, opening balance". Extract amount only. Same rule: do not judge eligibility, just extract.
  IMPORTANT for both: do NOT use for correcting an existing balance when the owner's OWN WORDS clearly indicate a correction (e.g. "set balance to X", "humne hisaab kiya, ab X baki hai", "change the opening balance to X") -- that is not supported yet. But absent such explicit correction language, always extract the action -- never withhold it based on a guess about the customer's transaction history.
